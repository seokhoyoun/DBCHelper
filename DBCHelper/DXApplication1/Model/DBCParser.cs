                ...
                using (StreamReader streamReader = new StreamReader(fileStream))
                {
                    ...
                    while (!streamReader.EndOfStream)
                    {
                        ...
                        if (rawLine.StartsWith(ATTRIBUTE_DEF_IDENTIFIER, COMPARISON) && !rawLine.StartsWith(ATTRIBUTE_DEF_DEFAULT_IDENTIFIER, COMPARISON))
                        {
                            ...
                            if (rawLine.Contains(NODE_IDENTIFIER) || rawLine.Contains(MESSAGE_IDENTIFIER) || rawLine.Contains(SIGNAL_IDENTIFIER))
                            {
                                ...
                                switch (attributeData[4])
                                {
                                    ...
                                    case "ENUM":
                                        ...
                                        string enumRawValue = rawLine.Split(new string[] { "ENUM" }, StringSplitOptions.RemoveEmptyEntries)[1];
                            ...
                            else
                            {
                                ...
                                switch (attributeData[3])
                                {
                                    ...
                                    case "ENUM":
                                        tempAttribute.AttributeValueType = EAttributeValueType.EnumType;
                                        string enumRawValue = rawLine.Split(new string[] { "ENUM" }, StringSplitOptions.None)[1];
                                        string[] enumValues = enumRawValue.Split(',');
                        ...
                        if (rawLine.StartsWith(VALUE_TABLE_IDENTIFIER, COMPARISON))
                        {
                            ...
                            string valueTableStr = rawLine.Split(new string[] { valueTableData[1] }, StringSplitOptions.RemoveEmptyEntries)[1];
                            ...
                            if (MessageDictionary.ContainsKey(messageID))
                            {
                                foreach (var signal in MessageDictionary[messageID].SignalList)
                                {
                                    if (signal.SignalName == tempSignalName)
                                    {
                                        ...
                                        string valueTableStr = rawLine.Split(new string[] { valueTableData[2] }, StringSplitOptions.RemoveEmptyEntries)[1];
                                        string[] decimalAndDescriptionArr = valueTableStr.Split(new char[] { '"', ';' }, StringSplitOptions.RemoveEmptyEntries);
                        ...
                        if (rawLine.StartsWith(ENVIRONMENT_VARIABLE_IDENTIFIER, COMPARISON))
                        {
                            ...
                            envInfo.Name = values[0].Replace(ENVIRONMENT_VARIABLE_IDENTIFIER, "").Trim();
                            ...
                            envInfo.Unit = values2[3].Replace("\"", "");
                            ...
                            envInfo.AccessNode = values2[7].Replace(";", "");
                        }
                    }
                }
            }
